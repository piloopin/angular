import {StringWrapper, isBlank, isString, compare} from 'angular2/src/facade/lang';
import {isListLikeIterable, ListWrapper} from 'angular2/src/facade/collection';
...
var _parser = new Parser(new Lexer(), reflector);
...
export class OrderByPipe extends Pipe {
  ...
  supports(obj) { return OrderByPipe.supportsObj(obj); }

  static _createMapper(expression) {
    var ast = _parser.parseBinding(expression, '');
    return function(e) { return ast.eval(e, null); };
  }

  static _buildCompareFn(expressions: List<any>, descending: boolean = false) {
    var numExpressions: int = expressions.length;
    var mappers: List<Function> = ListWrapper.createFixedSize(numExpressions);
    var cmpSign: List<int> = ListWrapper.createFixedSize(numExpressions);
    for (var i = 0; i < numExpressions; i++) {
      var expression = expressions[i];
      cmpSign[i] = descending ? -1 : 1;
      if (StringWrapper.isString(expression)) {
  ...
  transform(value, expression, descending: boolean = false) {
    value = ListWrapper.from(value);
    if (isBlank(expression)) return value;
    var compareFn;
    if (!ListWrapper.isList(expression)) {
...
export class OrderByPipeFactory implements PipeFactory {
  supports(obj): boolean { return OrderByPipe.supportsObj(obj); }

  create(cdRef): Pipe { return new OrderByPipe(); }
}
